...
fn iterate(
    ...
    if *best_geodes_at_timestep.get(&t).unwrap_or(&0) > resources.geode + 2 {
        return;
    }
    for i in t..=32 {
        if *best_geodes_at_timestep.get(&i).unwrap_or(&0) < resources.geode {
        ...
        .map(|s| s.parse::<Blueprint>().unwrap())
    {
        ...
        threads.push(std::thread::spawn(move || {
            ...
            iterate(
                ...
                local_max_geodes.clone(),
                &mut best_geodes_at_timestep,
            );
            if let Ok(lock) = local_max_geodes.lock() {
                println!("Done with blueprint {}!", blueprint.i);
                println!("{:?}\n", *lock);
            }
        }));
    }
