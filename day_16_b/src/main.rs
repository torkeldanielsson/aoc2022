...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let e_to_visit = to_visit.clone();

    for x in 4..=7 {
        for h_to_visit_v in to_visit.iter().combinations(x).unique() {
            let mut e_to_visit = e_to_visit.clone();
            let mut h_to_visit = std::collections::HashSet::new();
            for v in h_to_visit_v {
                h_to_visit.insert(*v);
                e_to_visit.remove(v);
            }

            let mut h_best_released_flow = 0;
            let mut e_best_released_flow = 0;
            ...
            solve(
                ...
                *index_map.get("AA").unwrap() as i32,
                ...
                &indexed_flow_map,
                e_to_visit,
                &mut e_best_released_flow,
            );

            if best_released_flow < h_best_released_flow + e_best_released_flow {
                best_released_flow = h_best_released_flow + e_best_released_flow;
            }
        }
    }
